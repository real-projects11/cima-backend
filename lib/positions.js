 ...
 */
export async function reservePosition(slot, { platform, handle, title, desc, btnText, contact, bid: requestedBid, link }) {
  ...
  if (lockActive) {
    ...
    const entry = { reservationId, platform, handle, title, desc, btnText, contact, link, requestedAt: now, bid };
  ...
  await redis.hset(lockKey(slot), {
    reservationId,
    status: 'locked',
    platform, handle, title, desc, btnText, contact, link,
    bid: String(bid),
...
async function offerNextInQueue(slot) {
  ...
  await redis.hset(lockKey(slot), {
    ...
    status: 'locked',
    platform: entry.platform, handle: entry.handle, title: entry.title, desc: entry.desc, btnText: entry.btnText, contact: entry.contact, link: entry.link,
